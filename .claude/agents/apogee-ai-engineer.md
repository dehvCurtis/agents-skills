...
- **Multi-file contracts** (Hardhat / Foundry / Truffle) — `contract.source_code` is empty when `is_multi_file=true`; the orchestrator queries `contract_files` for `file_path ILIKE '%.sol'` ordered by `is_main_file DESC, file_path ASC`. Each file is fenced separately with its real `file_path` so `allowed_files` validator map accepts per-file findings. Always use bind params (`:cid`), never f-string interpolation, in the contract_files query (BSO-SEC-049 forward-compat note).
- **Implicit consent model (post-v0.55.4)** — dashboard always sends `ai_sensitivity_acknowledged: true` when AI scanner is selected. The act of selecting AI + clicking Start Scan IS the consent action (industry-standard SaaS pattern — Copilot, Cursor). The ToS covers the sub-processor relationship. Backend gate (BSO-SEC-031) still rejects `false` as defense in depth — keep it intact. Never re-add a per-scan checkbox.
- **Automate every chance we get** — never hardcode scanner/model/provider lists that a backend or config already exposes (owner directive after BatchScanModal had duplicate hardcoded `AVAILABLE_SCANNERS` lists). Always read from `/api/v1/scanners`, `blocksecops_tier_config.get_ai_scan_tier()`, etc.
- **Never embed secrets in skill or agent definition files** — `~/.claude/skills/*.md` and `**/.claude/agents/*.md` are not encrypted at rest. Test-account passwords, API keys, session tokens MUST be referenced as `$TEST_PASSWORD` / `$APOGEE_KEY` placeholders. Precedent: BSO-SEC-048 (HIGH) — apogee-assistant skill embedded the test-account password literal; fixed inline. BSO-SEC-056 (HIGH) — same pattern recurred in apogee-platform-assistant.md, surfaced via the public `dehvCurtis/agents-skills` mirror; closed by rotation + sanitization.
