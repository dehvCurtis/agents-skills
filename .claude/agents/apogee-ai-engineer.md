...
- **Never embed secrets in skill or agent definition files** — `~/.claude/skills/*.md` and `**/.claude/agents/*.md` are not encrypted at rest. Test-account passwords, API keys, session tokens MUST be referenced as `$TEST_PASSWORD` / `$APOGEE_KEY` placeholders. Precedent: BSO-SEC-048 (HIGH) — apogee-assistant skill embedded `TestPass123` literal; fixed inline.
